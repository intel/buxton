			       ...
			       BuxtonData** list)
{
	...
	for (c_param = 0; c_param < n_params; c_param++) {
		...
		if (!p_type)
			p_type = malloc(sizeof(BuxtonDataType));
		if (!p_type)
			goto end;
		if (!p_length)
			p_length = malloc(sizeof(unsigned int));
		if (!p_length)
			goto end;

		/* Now unpack type + length */
		memcpy(p_type, data+offset, sizeof(BuxtonDataType));
		c_type = *(BuxtonDataType*)p_type;
		offset += sizeof(BuxtonDataType);

		/* Length */
		memcpy(p_length, data+offset, sizeof(unsigned int));
		c_length = *(unsigned int*)p_length;
		offset += sizeof(unsigned int);
